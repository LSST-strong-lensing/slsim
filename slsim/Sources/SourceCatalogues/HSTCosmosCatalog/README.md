...
Additionally, a source exclusion list is applied to filter out sources with contaminants in them. This source exclusion list was created by Sebastian Wagner-Carena. Currently, the three cuts outlined above are hardcoded and not flexible for change, since the source exclusion list would need to be modified to include additional sources that would not have been filtered out by the three cuts.
...
A notebook demonstrating how to match Sersics to real sources is given [here](https://github.com/LSST-strong-lensing/slsim/blob/main/notebooks/sersic_to_real_galaxy_source_matching.ipynb).