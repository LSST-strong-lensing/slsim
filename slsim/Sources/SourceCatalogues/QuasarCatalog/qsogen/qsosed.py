#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Code to produce model quasar SEDs. See accompanying README file for further
details.
...
def tau_eff(z):
    """Ly alpha optical depth from Becker et al.

    2013MNRAS.430.2067B.
    """
    tau_eff = 0.751 * ((1 + z) / (1 + 3.5)) ** 2.90 - 0.132
    return np.where(tau_eff < 0, 0.0, tau_eff)
...
class Quasar_sed:
    ...
    def wav2flux(self, wav):
        ...
        Different from self.flux[wav2num(wav)], as wav2flux interpolates
        in an attempt to avoid problems when wavlen has gaps. This
        mitigation only works before the emission lines are added to the
        model, and so wav2flux should only be used with a reasonably
        dense wavelength array.
        """
    ...
    def set_continuum(self, flxnrm=1.0, wavnrm=5500):
        ...
        self.flux = np.where(
            self.wavlen < wavbrk3, pl(self.wavlen, sl3, const3), fluxtemp
        )

    def add_blackbody(self, wnorm=20000.0):
    ...
    def add_balmer_continuum(
        self, tbc=15000.0, taube=1.0, wavbe=3646.0, wnorm=3000.0, vfwhm=5000.0
    ):
        ...
        winc = self.wavlen[self.wav2num(wnorm)] - self.wavlen[self.wav2num(wnorm) - 1]
    ...
    def add_emission_lines(self, wavnrm=5500, wmin=6000, wmax=7000):
        ...
        Emission-lines are included via 4 emission-line templates, which
        are packaged with a reference continuum. One of these templates
        gives the average line emission for a M_i=-27 SDSS DR16 quasar
        at z~2. The narrow optical lines have been isolated in a
        separate template to allow them to be re-scaled if necesssary.
        Two templates represent the observed extrema of the high-
        ionisation UV lines, with self.emline_type controlling the
        balance between strong, peaky, systemic emission and weak,
        highly skewed emission. Default is to let this vary as a
        function of redshift using self.beslope, which represents the
        Baldwin effect. The template scaling is specified by
        self.scal_emline, with positive values producing a scaling by
        intensity, whereas negative values give a scaling that preserves
        the equivalent-width of the lines relative to the reference
        continuum template. The facility to scale the H-alpha line by a
        multiple of the overall emission-line scaling is included
        through the parameter scal_halpha, and the ability to rescale
        the narrow [OIII], Hbeta, etc emission is included through
        scal_nlr.
        """
        ...
        if self.emline_type is None:
            if beslp:
                ...
                self.emline_type = 0.0  # default median emlines
        ...
        if varlin == 0.0:
            ...
            linval = medval + (scanlr - 1.0) * nlr
        elif varlin > 0:
            # high EW emission line template
            varlin = min(varlin, 3.0)
            linval = varlin * pkyval + (1 - varlin) * medval + (scanlr - 1.0) * nlr
        else:
            # highly blueshifted emission lines
            varlin = min(abs(varlin), 2.0)
            linval = varlin * wdyval + (1 - varlin) * medval + (scanlr - 1.0) * nlr

        # remove negative dips from extreme extrapolation (i.e. abs(varlin)>>1)
        linval[(linwav > 4930) & (linwav < 5030) & (linval < 0.0)] = 0.0
        linval[(linwav > 1150) & (linwav < 1200) & (linval < 0.0)] = 0.0
    ...
    def host_galaxy(self, gwnmin=4000.0, gwnmax=5000.0):
        ...
        vallum = self.M_i
        galnrm = -23.0  # this is value of M_i for gznorm~0.35
    ...
    def redden_spectrum(self, R=3.1):
        """Redden quasar component of total SED.
