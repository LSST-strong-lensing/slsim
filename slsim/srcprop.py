...
def dPhibydM_qso(redshift, mag, cosmo):
    ...
    fofz = (np.exp(zeta * redshift) * (1 + np.exp(xi * zstar))) / pow(
        np.sqrt(np.exp(xi * redshift)) + np.sqrt(np.exp(xi * zstar)), 2
    )
    ...
    return phistar / (
        pow(10, (0.4 * (alpha + 1) * (Mabs - Mstar)))
        + pow(10, (0.4 * (beta + 1) * (Mabs - Mstar)))
    )
...
def dNbydz_qso(zz, vdisp, zred, q, kwargs_source, cosmo, constants):
    ...
    return (
        Phi
        * 1.0
        / (cosmo.H(zz) / cosmo.H(0)).value
        * (constants.get("light_speed") / (cosmo.H(0).value))
        * (cosmo.angular_diameter_distance(zz).value) ** 2.0
        * csect
        * (1 + zz) ** 2
    )
...
def findzqso(ztry, vdisp, zred, q, Ntarget, kwargs_source, cosmo, constants):
    ...
    return (
        Ntarget
        - sci.quad(
            lambda zz: dNbydz_qso(zz, vdisp, zred, q, kwargs_source, cosmo, constants),
...
def findmagqso(magtry, zsrc, Phitarget, kwargs_source, cosmo):
    ...
    result = (
        Phitarget
        - sci.quad(
            lambda mag: dPhibydM_qso(zsrc, mag, cosmo),
            kwargs_source.get("min_mag"),
            magtry,
        )[0]
    )
...
def Nsrc_qso(magg, magr, magi, zred, q, vdisp, myseed, kwargs_source, cosmo, constants):
    ...
    fp1.write(
        "This lens has %f quasars behind it on average and Poisson deviate is %d\n"
    ...
    for ii in range(Nreal):
        ...
        while not done:
            try:
                ...
                print(
                    "checking",
                    findmagqso(kwargs_source.get("min_mag"), qsozsrc, Phitarg),
                    findmagqso(kwargs_source.get("max_mag"), qsozsrc, Phitarg),
                    kwargs_source.get("min_mag"),
                    kwargs_source.get("max_mag"),
                    Phitarg,
                )
...
def Nqso(kwargs_source, constants, cosmo):
    ...
    totNqso = sci.quad(
        lambda zz: dNqso(zz), kwargs_source.get("min_z"), kwargs_source.get("max_z")
    )[0]
...
def initnsmlim(kwargs_source):
    ...
    magi = np.arange(
        kwargs_source.get("min_mag"), kwargs_source.get("max_mag") + 0.01, 0.01
    )

    # Compute number densities
    nsi = np.array(
        [
            sci.quad(lambda xmag: dnsbydm(xmag), kwargs_source.get("min_mag"), mag)[0]
            for mag in magi
        ]
    )
...
def Nsrc_gal(magg, magr, magi, zred, q, vdisp, myseed, kwargs_source, cosmo, constants):
    ...
    Nsrcmean_boost = (
        Nsrcmean
        * nsmlim_gal_spl(kwargs_source.get("max_mag"))
        * kwargs_source.get("boost_csect")
    )
    ...
    for ii in range(Nreal):
        ...
        while not done:
            try:
                ...
                print(
                    "checking",
                    findmaggal(kwargs_source.get("min_mag"), galzsrc, Phitarg),
