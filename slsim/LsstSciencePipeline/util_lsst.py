...
from slsim.Util.param_util import (fits_append_table, convert_mjd_to_days,
                        transient_event_time_mjd, flux_error_to_magnitude_error,
                        transformmatrix_to_pixelscale, magnitude_to_amplitude)
import os
...
def optimized_transient_event_time_mjd(mjd_times, lightcurve_range=(-50, 100),
                                        min_points=100):
    """
    Finds a transient start time such that at least `min_points` fall within the lightcurve range.
    
    :param mjd_times: List or array of MJD times.
    :param lightcurve_range: Range of lightcurve in days (e.g., (-50, 100)).
    :param min_points: Minimum number of points required in the range. eg: 100
    :return: Chosen transient start mjd. Returns None if no valid start point is found.
    """
    mjd_times = np.array(mjd_times)
    min_mjd, max_mjd = min(mjd_times), max(mjd_times) 
    
    # Iterate through possible zero_point_mjd values
    for zero_point_mjd in np.linspace(min_mjd, max_mjd, 1000):  # Test multiple candidates
        start, end = zero_point_mjd + lightcurve_range[0],\
...
def transient_data_with_cadence(
    ...
    if start_point_mjd_time is None:
        start_point_mjd_time = transient_event_time_mjd(
                    ...
                    max(copied_exposure_data["obs_time"]))
    obs_time_in_days = convert_mjd_to_days(copied_exposure_data["obs_time"],
                                                start_point_mjd_time)
    ...
    lens_id = [lens_class.generate_id(ra=ra_dec_list[0].degree,
                                      dec=ra_dec_list[1].degree)]*len(
    ...
    for obs in range(len(copied_exposure_data["obs_time"])):
        ...
        magnitude = lens_class.point_source_magnitude(band=exposure_data_obs["band"],
                                                    lensed=True, time=observation_time)
        # computing necessary quantities in flux unit
        amplitude_image_1 = magnitude_to_amplitude(magnitude[0][0],
                                            exposure_data_obs["zero_point"])
        amplitude_image_2 = magnitude_to_amplitude(magnitude[0][1],
                                            exposure_data_obs["zero_point"])
        ...
        (mag_gaussian_realiz_1,  mag_err_1_minus,
          mag_err_1_plus) = flux_error_to_magnitude_error(
            amplitude_image_1, flux_err_1, exposure_data_obs["zero_point"],
              noise=noise, symmetric=symmetric)
        (mag_gaussian_realiz_2, mag_err_2_minus,
          mag_err_2_plus) = flux_error_to_magnitude_error(
            amplitude_image_2, flux_err_2, exposure_data_obs["zero_point"],
              noise=noise, symmetric=symmetric)
        ...
        if lens_class.image_number[0] > 2:
            amplitude_image_3 = magnitude_to_amplitude(magnitude[0][2],
                                            exposure_data_obs["zero_point"])
            ...
            (mag_gaussian_realiz_3,  mag_err_3_minus,
              mag_err_3_plus) = flux_error_to_magnitude_error(
            amplitude_image_3, flux_err_3, exposure_data_obs["zero_point"],
              noise=noise, symmetric=symmetric)
        ...
        if lens_class.image_number[0] > 3:
            amplitude_image_4 = magnitude_to_amplitude(magnitude[0][3],
                                            exposure_data_obs["zero_point"])
            ...
            (mag_gaussian_realiz_4,  mag_err_4_minus,
              mag_err_4_plus) = flux_error_to_magnitude_error(
            amplitude_image_4, flux_err_4, exposure_data_obs["zero_point"],
              noise=noise, symmetric=symmetric)
    ...
    magnitude_col_image_1 = Column(name="mag_image_1", data=np.array(
        magnitude_image_1).squeeze())
    magnitude_col_image_2 = Column(name="mag_image_2", data=np.array(
        magnitude_image_2).squeeze())
    magnitude_col_image_3 = Column(name="mag_image_3", data=np.array(
        magnitude_image_3).squeeze())
    magnitude_col_image_4 = Column(name="mag_image_4", data=np.array(
        magnitude_image_4).squeeze())
    total_mag_error_col_1_low = Column(name="mag_error_image_1_low", data=np.array(
        mag_error_1_minus).squeeze())
    total_mag_error_col_1_high = Column(name="mag_error_image_1_high", data=np.array(
        mag_error_1_plus).squeeze())
    total_mag_error_col_2_low = Column(name="mag_error_image_2_low", data=np.array(
        mag_error_2_minus).squeeze())
    total_mag_error_col_2_high = Column(name="mag_error_image_2_high", data=np.array(
        mag_error_2_plus).squeeze())
    total_mag_error_col_3_low = Column(name="mag_error_image_3_low", data=np.array(
        mag_error_3_minus).squeeze())
    total_mag_error_col_3_high = Column(name="mag_error_image_3_high", data=np.array(
        mag_error_3_plus).squeeze())
    total_mag_error_col_4_low = Column(name="mag_error_image_4_low", data=np.array(
        mag_error_4_minus).squeeze())
    total_mag_error_col_4_high = Column(name="mag_error_image_4_high", data=np.array(
        mag_error_4_plus).squeeze())
    image_list_col = Column(name="lens_image", data=image_list)
    # add these new quantities as new columns in the expo_data table.
    copied_exposure_data.add_columns([lens_id_col, magnitude_col_image_1, 
                    magnitude_col_image_2, magnitude_col_image_3, magnitude_col_image_4,
                    total_mag_error_col_1_low, total_mag_error_col_1_high, 
                    total_mag_error_col_2_low, total_mag_error_col_2_high, 
                    total_mag_error_col_3_low, total_mag_error_col_3_high, 
                    total_mag_error_col_4_low, total_mag_error_col_4_high, 
                    image_list_col])
    return copied_exposure_data

def extract_lightcurves_in_different_bands(data, images=True):
    """
    Extract lightcurves and images in different bands from the given catalog.
    :param data: An astropy table containing lightcurves in a certain cadence. 
     This table must contain magnitude and corresponding errors. 
     The column name for magnitude should be "mag_image_n", 
     and column names for the error should be 
     "mag_error_image_n_low" and "mag_error_image_n_high", where 
     n could be 1, 2, 3, or 4.
    :param images: Boolean, if True, extracts image lists by band. Default is True.
    :return: A dictionary of magnitudes, associated errors, observation times, 
     and optionally image lists, structured by image or band.
    """
    table = data
    ...
    errors_low = {f"mag_error_image_{i}_low": {band: [] for band in bands
                                               } for i in range(1, 5)}
    errors_high = {f"mag_error_image_{i}_high": {band: [] for band in bands
                                                 } for i in range(1, 5)}
