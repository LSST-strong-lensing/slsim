...
class Lens(LensedSystemBase):
    ...
    def point_source_magnitude(
        ...
        :type lensed: bool
        :param time: image observation time in days. If None, use the mean of
            each source's stored ``ps_mag_<band>`` value(s), then apply lens
            magnification if requested. This is normally a stored mean for
            quasars and the sampled peak for generated supernovae and
            kilonovae, unless a magnitude was supplied. No-time calls can
            generate transient light curves.
