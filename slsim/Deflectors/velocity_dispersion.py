...
def vel_disp_nfw_3d(r, m_halo, c_halo, cosmo, z_lens):
    ...
    s = r / r_halo
    vel2 = G.to('km2 Mpc / M_sun s2').value * m_halo / (r_halo * arcsec * lens_cosmo.dd)  # km^2 / s^2
    cs = c_halo * s
    g_c = 1 / (np.log(1 + c_halo) - c_halo / (1 + c_halo))
    vel_disp2 = vel2 * (
        1 / 2 * c_halo ** 2 * g_c * s * (1 + cs) ** 2 * (
        np.pi ** 2 - np.log(cs) - 1 / cs
        - 1 / (1 + cs) ** 2 - 6 / (1 + cs) + (1 + 1 / cs ** 2 - 4 / cs - 2 / (1 + cs)) *
        np.log(1 + cs) + 3 * np.log(1 + cs) ** 2 + 6 * spence(1 + cs)
        )
    )
...
def vel_disp_nfw_aperture(r, m_halo, c_halo, cosmo, z_lens):
    ...
    int1 = _log_integrate(lambda r_: vel_disp_nfw_3d(r_, m_halo, c_halo, cosmo, z_lens) ** 2 * r_ / r_halo / (
                1 + c_halo * r_ / r_halo) ** 2,
                         rmin, rmax)
    int2 = _log_integrate(
        lambda r_: (vel_disp_nfw_3d(r_, m_halo, c_halo, cosmo, z_lens) ** 2 / (1 + c_halo * r_ / r_halo) ** 2 *
