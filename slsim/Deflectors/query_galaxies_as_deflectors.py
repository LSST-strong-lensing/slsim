...
def find_potential_lenses(foreground_table, background_source, 
                          kwargs_deflector = {'min_z':0.2, 'max_z':1.9, 'min_shear':0.001,
                                              'max_shear':0.02, 'min_PA_shear':0.0, 'max_PA_shear':180.0}, 
                          kwargs_source = {'type':'galaxy', 'min_z':0.2, 'max_z':5.0, 'min_mag':25.0, 
                                           'max_mag':28.0, 'boost_csect':3, 'ell_min':0.1, 'ell_max': 0.8, 
                                           'PA_min': 0., 'PA_max':180., 'R_Einst_min': 0.5, 'R_Einst_max':3.0}, 
                          cosmo = FlatLambdaCDM(H0=72,Om0=0.26), constants = {'G':4.2994e-9, 'light_speed':299792.458}, 
                          parallel=False, save_source_file=False, chunk_offset=0):
    '''
     This function identifies potential lensing galaxies (deflectors) from a foreground galaxy catalog,
     on the basis of their angular cross section, source luminosity function and their magnitudes and 
     redshift falling within a certain bin, defined in the source and deflector kwargs.
    ...
    foreground_redshift, foreground_ellipticity = foreground_table['redshift'], foreground_table['ellipticity']
    foreground_gmag, foreground_rmag, foreground_imag = foreground_table['gmag'], foreground_table['rmag'], foreground_table['imag']
    foreground_err_g,foreground_err_r,foreground_err_i = foreground_table['err_g'], foreground_table['err_r'], foreground_table['err_i']
    ...
    foreground_velocity_dispersion = get_velocity_dispersion(deflector_type="elliptical", lsst_mags=lsst_mags, lsst_errs=lsst_errs, 
                                    redshift=foreground_redshift, cosmo=cosmo, bands=["g", "r", "i"], c1=0.01011, 
                                    c2=0.01920, c3=0.05162, c4=-0.00032, c5=0.06555, c6=-0.02949, c7=0.00003, 
                                    c8=0.04040, c9=-0.00892, c10=-0.03068, c11=-0.21527, c12=0.09394, 
                                    scaling_relation="spectroscopic")

    foreground_velocity_dispersion = unumpy.nominal_values(foreground_velocity_dispersion)
    ...
    for ii in range(len(foreground_table)):
        ...
        if(foreground_gmag[ii]>0 and foreground_rmag[ii]>0 and foreground_redshift[ii] >kwargs_deflector.get('min_z') and foreground_redshift[ii]<kwargs_deflector.get('max_z')):
            ...
            if(background_source=='galaxy'):
                background_source_counts,source_magnitude_array,source_redshift_array,randoms=srcprop.Nsrc_gal(foreground_gmag[ii],foreground_rmag[ii],foreground_imag[ii],foreground_redshift[ii],foreground_axisratio[ii], foreground_velocity_dispersion[ii], myseed,kwargs_source,cosmo,constants)
                
            elif(background_source=='quasar'):
                background_source_counts,source_magnitude_array,source_redshift_array,randoms=srcprop.Nsrc_qso(foreground_gmag[ii],foreground_rmag[ii],foreground_imag[ii],foreground_redshift[ii],foreground_axisratio[ii], foreground_velocity_dispersion[ii], myseed,kwargs_source,cosmo,constants)
            ...
            if(background_source_counts>0 and foreground_velocity_dispersion[ii]>0):
                for kk in range(background_source_counts):
                    ...
                    foreground_Einstein_radius=slp.getreinst(foreground_redshift[ii],source_redshift_array[kk],foreground_velocity_dispersion[ii],cosmo,constants)
                    ...
                    if(foreground_Einstein_radius*206264.8>=kwargs_source.get('R_Einst_min') and foreground_Einstein_radius*206264.8 <=kwargs_source.get('R_Einst_max') ):
                        sorted_indices.append(ii + chunk_offset)
                        sorted_velocity_dispersion.append(foreground_velocity_dispersion[ii])
                        sorted_Einstein_radius.append(foreground_Einstein_radius*206264.8)
    ...
    if parallel:
        ...
        return sorted_indices, sorted_velocity_dispersion, sorted_Einstein_radius, sorted_source_mag, sorted_source_redshift, count_source
    
    else:
        sorted_table = Table(
            [sorted_indices, sorted_velocity_dispersion, sorted_Einstein_radius, sorted_source_mag, sorted_source_redshift],
            names=('Id', 'Velocity dispersion', 'Einstein radius', 'Source mag', 'Source z')
        )
        ...
        foreground_sorted_table.write('sorted_galaxies.csv',format='csv',overwrite=True)

        return None

def find_potential_lenses_parallel(comm, rank, size, foreground_catalog, background_source, 
                                        kwargs_deflector = {'min_z':0.2, 'max_z':1.9, 'min_shear':0.001,
                                              'max_shear':0.02, 'min_PA_shear':0.0, 'max_PA_shear':180.0}, 
                                        kwargs_source = {'type':'galaxy', 'min_z':0.2, 'max_z':5.0, 'min_mag':25.0, 
                                           'max_mag':28.0, 'boost_csect':3, 'ell_min':0.1, 'ell_max': 0.8},  
    ...
    local_results = find_potential_lenses(local_chunk, background_source, kwargs_deflector, kwargs_source, 
                                          cosmo, constants, parallel='True', save_source_file=save_source_file, 
