...
from uncertainties import unumpy
from light2mass import get_velocity_dispersion
import numpy as np
...
def find_massive_ellipticals(DP0_table,fracDev_limit=0.8,cosmo=FlatLambdaCDM(H0=72, Om0=0.26),parallel=False):

    """
    This function identifies potential lensing galaxies (deflectors) from the 
    LSST DP0.2 data or any other catalog, based on the morphology, and provides their
    lens model parameters using the light-to-mass function in SLSim.
    ...
    id_valid = np.where( (~np.isnan (DP0_table['obj_u_cModelFlux'])) & (~np.isnan (DP0_table['obj_u_cModelFluxerr']) )
                        & (~np.isnan (DP0_table['obj_g_cModelFlux'])) & (~np.isnan (DP0_table['obj_g_cModelFluxerr']) )
                        & (~np.isnan (DP0_table['obj_r_cModelFlux'])) & (~np.isnan (DP0_table['obj_r_cModelFluxerr']) )
                        & (~np.isnan (DP0_table['obj_i_cModelFlux'])) & (~np.isnan (DP0_table['obj_i_cModelFluxerr']) )
                        & (~np.isnan (DP0_table['obj_z_cModelFlux'])) & (~np.isnan (DP0_table['obj_z_cModelFluxerr']) )
                        & (~np.isnan (DP0_table['obj_y_cModelFlux'])) & (~np.isnan (DP0_table['obj_y_cModelFluxerr']) )
    ...
    id_ellipticals = np.where( (np.array(fracDev_list) > fracDev_limit) & (np.array(fracDev_list) <= 1.2) )[0]
    ...
    Mag_r_sdss = sdss_mags[2] - k_correction_coefficients[:,2] - 5.0 * np.log10(luminosity_distance/10)
    ...
    massive_ellipticals_velocity_dispersion = get_velocity_dispersion(deflector_type='elliptical', 
    ...
    DP0_massive_ellipticals['velocity dispersion'] = massive_ellipticals_velocity_dispersion
