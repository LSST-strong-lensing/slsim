...
class NFWCluster(DeflectorBase):
    ...
    - 'core': boolean flag for cored density profile
    """
    ...
    def _halo_mass_model_lenstronomy(self, lens_cosmo, spherical=False):
        ...
        if cored:
            ...
            r_cmax = 0.3 * rs_halo  # assuming Rs in NFW could be used for PSEUDOJAFFE
            r_c = np.random.uniform(r_cmin, r_cmax)
            vel_disp = self.velocity_dispersion()
            sigma0 = lens_cosmo.vel_disp_dPIED_sigma0(vel_disp, r_c, rs_halo)
