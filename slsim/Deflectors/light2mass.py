...
option to decide which scaling relations he wants to use, the one derived from spectroscopic
measurements or from the weak lensing measurements.
'''

def get_velocity_dispersion(deflector_type,
                            lsst_mags,
                            lsst_errs, 
                            zz='zz',cosmo='cosmo',bands=['g','r','i'],
                            c1=0.01011,c2=0.01920,c3=0.05162,c4=-0.00032,
                            c5=0.06555,c6=-0.02949,c7=0.00003,c8=0.04040,c9=-0.00892,
                            c10=-0.03068,c11=-0.21527,c12=0.09394,scaling_relation='spectroscopic'):

    '''
    input_params:

    deflector_type: type of the foreground/ deflector, e.g., 'galaxy-elliptical'
    type: string
    ...
    zz:   a 1D array of the redshifts
    ...
    if(deflector_type !='galaxy-elliptical'):
    ...
    for ind in range(len(indices)):
        lsst['{0}'.format(lsst_bands[indices[ind]])] = unumpy.uarray(lsst_mags[:,ind],lsst_errs[:,ind])
        sdss_responses.append('sdss_%s0'%(lsst_bands[indices[ind]]))

    #transform from lsst to sdss magnitudes
    mgSDSS = lsst['g'] + c1 +  (c2*(lsst['g']-lsst['r']))  +  (c3*(lsst['g']-lsst['r'])**2)
    mrSDSS = lsst['r'] + c4 +  (c5*(lsst['g']-lsst['r']))  +  (c6*(lsst['g']-lsst['r'])**2)
    miSDSS = lsst['i'] + c7 +  (c8*(lsst['r']-lsst['i']))  +  (c9*(lsst['r']-lsst['i'])**2)
    ...
    if(scaling_relation=='spectroscopic'):
        ...
        k_corrections = kcorr_sdss(np.array([mgSDSS,mrSDSS,miSDSS]),
                                    zz,responses=sdss_responses,responses_out=sdss_responses,band_shift=0.0)
        
    elif(scaling_relation=='weak-lensing'):
        # k-correct upto redshift z=0.1, since the scaling relations used are at z=0.1
        k_corrections = kcorr_sdss(np.array([mgSDSS,mrSDSS,miSDSS]),
                                    zz,responses=sdss_responses,responses_out=sdss_responses,band_shift=0.1)
    ...
    Dlum=  (cosmo.luminosity_distance(zz)*cosmo.H(0)/100).value

    #Convert the apparent r and B-band magnitudes to the absolute r and B-band magnitudes using the redshift and cosmology
    #defined
    ...
    if(scaling_relation=='spectroscopic'):
        ...
        the B-band luminosity function evolves such that characteristic 
        magnitude MBstar decline by 1.5 magnitudes from z=0.0 to z=1.0.
        We use the same assumption here; 

        Hence, MBstar and zz should follow the relation, i.e., 
                                MBstar = MBstar0-(zz)*1.5.
        where MBstar0 = MBstar(at zz=0). 
        In our case, MBstar0 = -19.31 has been estimated from the mean value of the MBstar0, from
        ...
        '''We assume the same assumption here (from Bell et al 2004) for decline of characteristic 
        magnitude Mrstar for r'-band,

        Hence, Mrstar and zz should follow the relation, i.e., 
                                MBstar = MBstar0-(zz-0.1)*1.5.
        where Mrstar0 = MBstar(at zz=0.1). 

        In our case, Mrstar0 = -20.44 has been estimated from Table 2, Blanton et al 2003.
        '''

        # Convert the sdss r-mag to r'-mag from Frei & Gunn 2003.
