...
def additional_poisson_noise_with_rescaled_coadd(image, original_exp_time,
                                         degraded_exp_time, use_noise_diff=True):
    """
    Computes additional Poisson noise to an image based on the change in exposure time.

    :param image : numpy.ndarray
        The input image array.
    :param original_exp_time : numpy.ndarray
        The original exposure time per pixel.
    :param degraded_exp_time : numpy.ndarray
        The degraded exposure time per pixel.
    :param use_noise_diff : bool, optional
        If True, approximates noise difference using Gaussian noise,
        otherwise, applies Poisson sampling. Default is True.
    :return: numpy.ndarray
        The additional noise to be added to the image.
    """
    ...
    if use_noise_diff:
        sigma_add = np.where(
            original_exp_time > 0, 
            np.sqrt(image_positive) * np.sqrt(1 / degraded_exp_time - 1 / original_exp_time),
            0.0
        )
        ...
        image_with_poisson = np.where(
            original_exp_time > 0,
            np.random.poisson(lam=image_positive * degraded_exp_time) / degraded_exp_time,
            0.0
        )
        return image_with_poisson - image_positive

def additional_bkg_rms_with_rescaled_coadd(image, original_rms, degraded_rms, use_noise_diff=True):
    """
    Computes additinal background noise based on RMS values before and after degradation.

    :param image : numpy.ndarray
        The input image array.
    :param original_rms : float
        The original root mean square (RMS) noise.
    :param degraded_rms : float
        The degraded RMS noise.
    :param use_noise_diff : bool, optional
        If True, approximates noise difference using Gaussian noise,
        otherwise, applies new Gaussian noise directly. Default is True.
    :return: numpy.ndarray
        The additional noise to be added to the image.
    """
    if use_noise_diff:
...
def degrade_coadd_data(image, variance_map, exposure_map, original_num_years=5,
                        degraded_num_years=1, use_noise_diff=True):
    """
    ...
    :return: The degraded image, the new variance map, and he new exposure map.
    """
    ...
    degraded_image += additional_bkg_rms_with_rescaled_coadd(image, original_rms,
                                                         degraded_rms, use_noise_diff)
