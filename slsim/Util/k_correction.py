...
def kcorr_sdss(
    ...
    """
    Computes the astronomical K correction for galaxies on the SDSS broadband 
    filters using the kcorrect module based on Blanton and Roweis 2007.

    :param mags_sdss: numpy.ndarray 
        The multi-band SDSS magnitudes of all the targets. A 2D array of 
        uncertainties.core.Variable, with bands along the rows and targets 
        along the columns.
        
        If your magnitude is m1 and associated error is e1, 
        uncertainties.core.Variable should be: ufloat(m1, e1).
    :param redshift: numpy.ndarray 
        An array of the redshifts of the deflectors. A 1D array of floats.
    :param responses: list of str 
        The SDSS bands for which the magnitude is provided.
    :param responses_out: list of str 
        The SDSS bands on which you want the K-corrections.
    :return: numpy.ndarray 
        A 2-D array with K-correction for all the targets, with each row 
        containing the K-correction for output bands for each target.
    """
