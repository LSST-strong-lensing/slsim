...
def kcorr_sdss(mags_sdss,redshift,responses=['sdss_u0','sdss_g0','sdss_r0','sdss_i0','sdss_z0'],
               responses_out = ['sdss_u0','sdss_g0','sdss_r0','sdss_i0','sdss_z0'],band_shift=0.0):
    ...
    all_b0 = np.array([1.4e-10, 0.9e-10, 1.2e-10, 1.8e-10, 7.4e-10],
                  dtype=np.float32)
    ...
    kc = kcorrect.kcorrect.Kcorrect(responses=responses,responses_out=responses_out,redshift_range=[0,2])
    ...
    for j in range(len(maggies)):
        for k in np.arange(len(responses), dtype=int):
            maggies[j, k] = 2. * b0[k] * np.sinh(- np.log(b0[k])
                                                - (0.4 * np.log(10.)
                                                    * mags[j, k]))
            maggies_err = (2. * b0[k] * np.cosh(- np.log(b0[k])
                                                            - (0.4 * np.log(10.) *
                                                            mags[j, k])) *
                                    0.4 * np.log10(10.) * mag_errs[j, k])
            maggies_ivar[j, k] = 1. / maggies_err**2
