...
def SDSS_to_LSST(sdss_u, sdss_g, sdss_r, sdss_i, sdss_z):
    """Converts the five bands SDSS magnitudes to the LSST magnitudes using
    empirically derived transformations. These transformations are only valid
    for red galaxies and should not be used for blue galaxies or other stellar
    populations. The coefficients are based on polynomial fits to color terms.
    ...
    lsst_u = (
        sdss_u
        + 0.008274
        + (-0.128901 * (sdss_u - sdss_g))
        + (0.021774 * (sdss_u - sdss_g) ** 2)
    )
    lsst_g = (
        sdss_g
        - 0.007948
        + (-0.025930 * (sdss_g - sdss_r))
        + (-0.031108 * (sdss_g - sdss_r) ** 2)
    )
    lsst_r = (
        sdss_r
        + 0.001010
        + (-0.013431 * (sdss_r - sdss_i))
        + (-0.024717 * (sdss_r - sdss_i) ** 2)
    )
    lsst_i = (
        sdss_i
        + 0.001282
        + (-0.039931 * (sdss_i - sdss_z))
        + (0.011767 * (sdss_i - sdss_z) ** 2)
    )
    lsst_z = (
        sdss_z
        + 0.012325
        + (0.191612 * (sdss_i - sdss_z))
        + (-0.053284 * (sdss_i - sdss_z) ** 2)
    )
