...
class Lens(LensedSystemBase):
    ...
    def extended_source_postion_list(self):
        """Provides extended source position list """
        if not hasattr(self, "_source_pos_list"):
            self._source_pos_list = []
            for index in range(len(self._source)):
                single_source_pos = self._extended_source_position(index)
                self._source_pos_list.append(single_source_pos)
        return self._source_pos_list
    
    def point_source_postion_list(self):
        """Provides point source position list"""
        if not hasattr(self, "_point_source_pos_list"):
            self._point_source_pos_list = []
            for index in range(len(self._source)):
                single_point_source_pos = self._point_source_position(index)
                self._point_source_pos_list.append(single_point_source_pos)
        return self._point_source_pos_list

    def _extended_source_position(self, index):
        ...
        """
        source_pos_x, source_pos_y = self.source(index).extended_source_position
        ...
        center_source = np.array([source_pos_x, source_pos_y])
        return center_source
    
    def _point_source_position(self, index):
        ...
        extended_source_center = self.extended_source_postion_list()[index]

        # Apply offsets if provided
        if self.source(index=index).point_source_offset[0] is not None:
            center_x_point_source = extended_source_center[0] + self.source(
                index=index).point_source_offset[0]
            center_y_point_source = extended_source_center[1] + self.source(
                index=index).point_source_offset[1]
        else:
        ...
        if not hasattr(self, "_es_image_position_list"):
            ...
            for index in range(len(self._source)):
                self._es_image_position_list.append(
                    self._extended_source_image_positions(index=index)
                )
        return self._es_image_position_list

    def _extended_source_image_positions(self, index):
        """Returns extended source image positions by solving the lens equation
        for a single source.
        ...
        lens_model_class = LensModel(
            ...
            multi_plane=False,
            z_source=self.source(index=index).redshift,
        )
        lens_eq_solver = LensEquationSolver(lens_model_class)
        source_pos_x, source_pos_y = self.extended_source_postion_list()[index]
        if (
            ...
            solver = "lenstronomy"
        einstein_radius = self.einstein_radius[index]
        ...
        if not hasattr(self, "_ps_image_position_list"):
            ...
            for index in range(len(self._source)):
                self._ps_image_position_list.append(
                    self._point_source_image_positions(index=index)
                )
        return self._ps_image_position_list

    def _point_source_image_positions(self, index):
        """Returns point source image positions by solving the lens equation
        ...
        lens_eq_solver = LensEquationSolver(lens_model_class)
        point_source_pos_x, point_source_pos_y = self.point_source_postion_list()[index]
    ...
    def _validity_test(
        ...
        z_lens = self.deflector.redshift
        z_source = self.source(index=source_index).redshift
        if z_lens >= z_source:
            return False
        ...
        if self.source(index=source_index).source_type in ["extended"]:
            source_pos = self.extended_source_postion_list()[source_index]
        elif self.source(index=source_index).source_type in ["point_source", "point_plus_extended"]:
            source_pos = self.point_source_postion_list()[source_index]
        ...
        if mag_arc_limit is not None and self.source(index=source_index).source_type in [
            "extended",
            "point_plus_extended",
        ]:
            ...
            host_mag = self._extended_single_source_magnification(index=source_index)
            for band, mag_limit_band in mag_arc_limit.items():
                mag_source = self._extended_source_magnitude(band, index= source_index)
    ...
    def einstein_radius(self):
        ...
        if not hasattr(self, "_theta_E_list"):
            self._theta_E_list = []
            for index in range(len(self._source)):
                self._theta_E_list.append(self._einstein_radius(index=index))
        return self._theta_E_list

    def _einstein_radius(self, index):
        ...
        """
        if self.deflector.redshift >= self.source(index=index).redshift:
            theta_E = 0
            return theta_E
        lens_model_list, kwargs_lens = self.deflector_mass_model_lenstronomy()
        ...
        if self.deflector.deflector_type in ["EPL"]:
            ...
            if self.source(index=index).redshift == self.max_redshift_source_class.redshift:
                ...
                beta = self._lens_cosmo.beta_double_source_plane(
                    ...
                    z_source_1=self.source(index=index).redshift,
                )
    ...
    def _point_source_arrival_times(self, index):
        """Arrival time of images relative to a straight line without lensing.
        Negative values correspond to images arriving earlier, and positive
        ...
        x_image, y_image = self._point_source_image_positions(index=index)
    ...
    def _image_observer_times(self, index, t_obs):
        ...
        """
        arrival_times = self._point_source_arrival_times(index=index)
    ...
    def point_source_magnitude(self, band, lensed=False, time=None):
        ...
        for index in range(len(self._source)):
            magnitude_list.append(
                self._point_source_magnitude(band, index, lensed=lensed, time=time)
            )
        return magnitude_list

    def _point_source_magnitude(self, band, index, lensed=False, time=None):
        ...
        if lensed:
            magnif = self._point_source_magnification(index=index)
            magnif_log = 2.5 * np.log10(abs(magnif))
            if time is not None:
                time = time
                image_observed_times = self._image_observer_times(index, time)
                variable_magnitude = self.source(index=index).point_source_magnitude(
                    band,
                ...
                source_mag_unlensed = self.source(index=index).point_source_magnitude(band)
                magnified_mag_list = []
        ...
        return self.source(index=index).point_source_magnitude(band)
    ...
    def _extended_source_magnitude_for_each_image(self, band, index, lensed=False):
        ...
        """
        if lensed:
            magnif = self._point_source_magnification(index=index, extended=True)
            magnif_log = 2.5 * np.log10(abs(magnif))
            source_mag_unlensed = self.source(index=index).extended_source_magnitude(band)
            magnified_mag_list = []
            for i in range(len(magnif_log)):
                magnified_mag_list.append(source_mag_unlensed - magnif_log[i])
            return np.array(magnified_mag_list)
        return self.source(index=index).extended_source_magnitude(band)

    def _extended_source_magnitude(self, band, index, lensed=False):
        """Unlensed apparent magnitude of the extended source for a given band
        (assumes that size is the same for different bands). This function
        ...
        # band_string = str("mag_" + band)
        # TODO: might have to change conventions between extended and point source
        source_mag = self.source(index=index).extended_source_magnitude(band)
        if lensed:
            mag = self._extended_single_source_magnification(index=index)
        ...
        if not hasattr(self, "_ps_magnification_list"):
            ...
            for index in range(len(self._source)):
                self._ps_magnification_list.append(
                    self._point_source_magnification(index=index)
                )
        return self._ps_magnification_list

    def _point_source_magnification(self, index, extended=False):
        ...
        if extended is True:
            img_x, img_y = self.extended_source_image_positions()[index]
        else:
            img_x, img_y = self.point_source_image_positions()[index]
        ...
        if not hasattr(self, "_extended_source_magnification_list"):
            ...
            for index in range(len(self._source)):
                self._extended_source_magnification_list.append(
                    self._extended_single_source_magnification(index)
                )
        ...
        if not hasattr(self, "_es_magnification_for_each_image_list"):
            ...
            for index in range(len(self._source)):
                self._es_magnification_for_each_image_list.append(
                    self._point_source_magnification(index, extended=True)
                )
        return self._es_magnification_for_each_image_list

    def _extended_single_source_magnification(self, index):
        """Compute the extended lensed surface brightness and calculates the
        ...
        light_model_list = self.source(index=index).extended_source_light_model()
        kwargs_source_mag = self.source(index=index).kwargs_extended_source_light(
        )
        #make sure that source center is not None
        kwargs_source_mag[0]["center_x"] = self.extended_source_postion_list()[index][0]
        kwargs_source_mag[0]["center_y"] = self.extended_source_postion_list()[index][1]
        ...
        theta_E = self.einstein_radius[index]
        center_source = self.extended_source_postion_list()[index]
    ...
    def source_light_model_lenstronomy(self, band=None):
        ...
        if (
            ...
            for index, source in enumerate(self._source):
                ...
                # Make sure source centers are not None. source class provides None if not provided
                kwargs_es_light[0]["center_x"] = self.extended_source_postion_list()[index][0]
                kwargs_es_light[0]["center_y"] = self.extended_source_postion_list()[index][1]
        ...
        if (
            ...
            for index in range(len(self._source)):
                ...
                img_x, img_y = self._point_source_image_positions(index=index)
                if band is None:
                    image_magnitudes = np.abs(self._point_source_magnification(index=index))
                else:
                    image_magnitudes = self._point_source_magnitude(
                        band=band, index=index, lensed=True
                    )
