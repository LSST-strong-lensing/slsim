...
class Lens(LensedSystemBase):
    ...
    def extended_source_image_positions(self):
        """Returns extended source image positions by solving the lens equation for each
        source.
    ...
    def _extended_source_image_positions(self, source):
        ...
        :param source: Source class instance. The redshift of this source is used in the
            LensModel.
        :return: x-pos, y-pos
        """
    ...
    def einstein_radius(self):
        ...
        if not hasattr(self, "_theta_E_corrected_list"):
            ...
            for source in self.source:
                self._theta_E_corrected_list.append(
                    self._einstein_radius(source=source)
                )
    ...
    def image_observer_times(self, t_obs):
        ...
        :return: time of the source when seen in the different images (without redshift
            correction)
        :rtype: list of numpy array. Each element of the array corresponds to different
    ...
    def point_source_magnitude(self, band, lensed=False, time=None):
        """Point source magnitudes, either unlensed (single value) or lensed (array)
        with macro-model magnifications. This function provided magnitudes of all the
        sources.
    ...
    def _point_source_magnitude(self, band, source, lensed=False, time=None):
        ...
        macro-model magnifications. This function does operation only for the single
        source.
    ...
    def point_source_magnification(self):
        ...
        :return: list of signed magnification of point sources in same order as image
            positions.
        """
    ...
    def _extended_single_source_magnification(self, source, source_index):
        ...
        _light_model_list = kwargs_model.get("source_light_model_list", [])[
            source_index
        ]
