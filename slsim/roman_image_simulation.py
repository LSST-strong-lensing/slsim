...
def simulate_roman_image(
    ...
    **kwargs,
    ...
    image = galsim_image_processing(image_array=array, band=band, num_pix=num_pix,
                psf_directory=psf_directory, oversample=oversample, detector=detector,
...
def galsim_image_processing(image_array, band, num_pix, psf_directory, oversample, detector,
                            detector_pos):
    """This function processes given image through the galsim InterpolatedImage class and
     convolves with the given psf.

    :param image_array: 2d array of an image.
    :param band: imaging band
    :type band: string
    :param num_pix: number of pixels per axis
    :param psf_directory: Path to directory containing psf file(s) where the psf can be loaded.
    ...
    :param detector_pos: The position of the detector being used to generate the psf
    :type detector_pos: integer between 4 + num_pix * oversample and 4092 - num_pix * oversample
    :return: 2d array of an image convolved with given psf.
    """
    # Converts image to the galsim InterpolatedImage class
    interp = InterpolatedImage(
        Image(image_array, xmin=0, ymin=0),
        scale=0.11 / oversample,
        flux=np.sum(image_array),
    )
...
def lens_image_roman(
    ...
    **kwargs
    ...
    convolved_deflector = galsim_image_processing(image_array=deflector_image, band=band,
                        num_pix=num_pix, psf_directory=psf_directory, oversample=oversample,
                          detector=detector, detector_pos=detector_pos)
    ...
    final_image = final_image / _exposure_time
    return final_image