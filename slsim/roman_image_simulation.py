...
def simulate_roman_image(
    ...
    **kwargs
    ...
    sim_api = SimAPI(
        numpix=num_pix*oversample, kwargs_single_band=kwargs_single_band, kwargs_model=kwargs_model
    )
    ...
    total_flux_cps = calculate_total_flux_cps(image_model, kwargs_source, kwargs_lens_light)

    # Converts image to the galsim InterpolatedImage class
    interp = InterpolatedImage(Image(array, xmin=0, ymin=0), scale=0.11 / oversample,
    ...
    if add_noise:
        ...
        image = add_roman_background(image, band, detector, num_pix, _exposure_time, ra, dec, date)

        # Add detector effects and get the resulting array
        rng = galsim.UniformDeviate(seed)
        roman.allDetectorEffects(image, prev_exposures=(), rng=rng, exptime=_exposure_time)
...
def calculate_total_flux_cps(image_model, kwargs_source, kwargs_lens_light):
    ...
    flux_lens_light = image_model.LensLightModel.total_flux(kwargs_list=kwargs_lens_light)
...
def get_psf(band, detector, detector_pos, oversample, psf_directory):
    ...
    psf_file_name = f'{band}_{wfi.detector}_{detector_pos[0]}_{detector_pos[1]}_{oversample}.pkl'
    if psf_directory is not None:
        psf_file_path = os.path.join(psf_directory, psf_file_name)
    else:
        psf_file_path = os.path.join(os.path.dirname(__file__), '..', "data", "webbpsf", psf_file_name)
...
def add_roman_background(image, band, detector, num_pix, exposure_time, ra, dec, date):
    """
    Adds a sky and thermal background to image, corresponding to a specific band, detector,
    date, and coordinate in the sky
