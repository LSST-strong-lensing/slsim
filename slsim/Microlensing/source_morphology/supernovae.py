...
class SupernovaeSourceMorphology(SourceMorphology):
    ...
    def _continuous_monochromatic_morphology(self, wavelength_angstroms, r_base_meters):
        ...
        ring_intensity = (R_eff / r_phot)**2 * np.exp(-(R_eff / (r_phot * 0.8))**4)
        
        intensity = (1.0 - ring_fraction) * continuum_intensity + ring_fraction * ring_intensity
        intensity[R_eff > r_phot] = 0.0
        
        return intensity, max_scale_m

    def get_kernel_map(self):
        """Builds the SED-weighted kernel map and sets the current physical pixel scale."""
        ...
        for wave, flux, trans in zip(wavelengths, sed_flux, transmissions):
            if trans < 0.01 or flux <= 0:
                continue
            spatial_profile, max_scale_m = self._continuous_monochromatic_morphology(wave, r_base_meters)
    ...
    def get_time_dependent_kernel_maps(self, time_anchors_days):
        ...
        Returns the kernels and their corresponding physical pixel scale in meters.
        """
        kernels = []
        pixel_scales_m = []
