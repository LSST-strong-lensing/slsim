...
class MicrolensingLightCurveFromLensModel(object):
    """Class to generate microlensing lightcurves based on the lens class
    provided."""
    ...
    def generate_point_source_microlensing_magnitudes(
        ...
        for agn point sources. For single source only, it produces the
    ...
    def _generate_agn_lightcurve(
        ...
        """
        lightcurve_time = (
            source_class.lightcurve_time
        )  # array of times (days) at which the lightcurve is needed
        lightcurve_duration = np.max(lightcurve_time) - np.min(
            lightcurve_time
        )  # in days
        ...
        image_positions_x, image_positions_y = (
            self.lens_class._point_source_image_positions(source_class)
        )
