...
class MicrolensingLightCurve(object):
    ...
    def _generate_point_source_lightcurve(
        ...
        """Generate lightcurves for one single point source with certain size, but for all
        ...
        if "source_size" in kwargs_PointSource:
            source_size = kwargs_PointSource["source_size"] #TODO: Make sure this is supplied in arc sec units
        else:
            raise ValueError(
        ...
        xs = np.linspace(-self.magnification_map.half_length_x,
                         self.magnification_map.half_length_x,
                         self.magnification_map.num_pixels_x)
        ys = np.linspace(-self.magnification_map.half_length_y,
                         self.magnification_map.half_length_y,
                         self.magnification_map.num_pixels_y)
        X, Y = np.meshgrid(xs, ys)
        # use gaussian profile to create the source size map
        sigma = source_size
        source_size_gaussian_map = (1/np.sqrt(2*np.pi*sigma**2)) * np.exp(-((X**2 + Y**2) / (2 * sigma**2)))
        
        # convolve the magnification map with the source size map
        ...
        if "effective_transverse_velocity" in kwargs_PointSource:
            effective_transverse_velocity = kwargs_PointSource["effective_transverse_velocity"] #TODO: Make sure this is supplied in km/s units
        else:
            raise ValueError(
        ...
        light_curve_time_in_years = self.time_duration / 365.25 # converting time_duration from days to years
        
        # extract_light_curve function requires the pixel size in meters
        pixel_size_arcsec = self.magnification_map.pixel_size # TODO: Is kpc_proper_per_arcmin the correct conversion factor?
        pixel_size_kpc = ((cosmology.kpc_proper_per_arcmin(source_redshift)).to(u.kpc/u.arcsec)) * pixel_size_arcsec * u.arcsec
        ...
        light_curves_list = [] # list of light curves       # has a shape of (num_lightcurves, length of light curve)
        tracks = [] # list of [x_positions,y_positions]     # has a shape of (num_lightcurves, 2, length of light curve)
        ...
        for i in range(num_lightcurves):
            ...
            macro_magnification = self.magnification_map.mu_ave

            if lightcurve_type == "magnitude":
                light_curve = -2.5 * np.log10(light_curve / np.abs(macro_magnification))
        ...
        light_curves_list = np.array(light_curves_list) #has a shape of (num_lightcurves, length of light curve)
        tracks = np.array(tracks) #has a shape of (num_lightcurves, 2, length of light curve)
...
class MicrolensingLightCurveFromLensModel(object):
    ...
    def generate_point_source_microlensing_magnitudes(
        ...
        Returns a numpy array of microlensing magnitudes with the shape
        (num_images, len(time)).
        """
        # if time is a number 
        ...
        lightcurves_single = np.zeros((len(lightcurves), len(time_array))) #has shape (num_images, len(time))
        for i in range(len(lightcurves)):
            lightcurves_single[i] = lightcurves[i][0]

        if np.isinstance(time, (int, float)):
    ...
    def _generate_point_source_lightcurve(
        ...
        if (np.isinstance(time, np.array) or np.isinstance(time, list)) and len(time) > 1:
    ...
    def generate_magnification_maps_from_microlensing_params(
        ...
        """
        Generate magnification maps for each image of the source based on the image positions
        and the lens model. It requires the following parameters:
    ...
    def _generate_agn_lightcurve(
        ...
        elif np.isinstance(time, np.array) or np.isinstance(time, list) and len(time) == 1:
