...
class SourceMorphology:
    ...
    def get_kernel_map(self, *args, **kwargs):
        ...
        The kernel map is a 2D array that represents the morphology of the source.
        The kernel map is used to convolve with the microlensing magnification map.
        """
    ...
    def pixel_scale_x_m(self):
        ...
        if not hasattr(self, "_pixel_scale_x_m"):
            self._pixel_scale_x_m = self.arcsecs_to_metres(self.pixel_scale_x, self.cosmo, self.source_redshift)
    ...
    def pixel_scale_y_m(self):
        ...
        if not hasattr(self, "_pixel_scale_y_m"):
            self._pixel_scale_y_m = self.arcsecs_to_metres(self.pixel_scale_y, self.cosmo, self.source_redshift)
    ...
    def pixel_scale_m(self):
        ...
        if not hasattr(self, "_pixel_scale_m"):
            self._pixel_scale_m = self.arcsecs_to_metres(self.pixel_scale, self.cosmo, self.source_redshift)
    ...
    def arcsecs_to_metres(self, arcsecs, cosmo, redshift):
        ...
        :return: Transverse distance in meters in the source plane at the given redshift.
        """
        # Convert arcseconds to radians
        radians = (arcsecs* u.arcsec.to(u.rad)).value
        # Calculate the angular diameter distance in meters
        angular_diameter_distance = (cosmo.angular_diameter_distance(redshift).to(u.m)).value
...
class GaussianSourceMorphology(SourceMorphology):
    ...
    def __init__(self, 
        ...
        :param length_x: Length of the kernel map in x direction in arcseconds. Make sure this is larger than the source size.
        :param length_y: Length of the kernel map in y direction in arcseconds.
        :param num_pix_x: Number of pixels in x direction.
        :param num_pix_y: Number of pixels in y direction.
        :param center_x: Center of the kernel map in x direction in arcseconds.
        :param center_y: Center of the kernel map in y direction in arcseconds.
    ...
    def get_kernel_map(self):
        """
        Returns the 2D array of the Gaussian kernel map.
        The kernel map is a 2D array that represents the morphology of the source.
        The kernel map is used to convolve with the microlensing magnification map.
        The kernel is normalized to 1.
...
class AGNSourceMorphology(SourceMorphology):
    ...
    def __init__(self,
        ...
        :param black_hole_spin: The dimensionless spin parameter of the black hole, where the spinless case (spin = 0) corresponds to a Schwarzschild black hole. Positive spin represents the accretion disk's angular momentum is aligned with the black hole's spin, and negative spin represents retrograde accretion flow.
        ...
        #-----------------------------------------------------------        
        gravitational_radius_of_smbh = calculate_gravitational_radius(self.smbh_mass_exp)
        ...
        self._pixel_scale_x_m = kernel_pixel_size_x_metres.to(u.m).value  # convert to meters
        self._pixel_scale_y_m = kernel_pixel_size_y_metres.to(u.m).value

        self._pixel_scale_m = np.sqrt(
            self._pixel_scale_x_m * self._pixel_scale_y_m
        )
        #----------------------------------------------------------- 
        ...
        self._pixel_scale = np.sqrt(
            self._pixel_scale_x * self._pixel_scale_y
        )
        #-----------------------------------------------------------
    ...
    def get_kernel_map(self):
        """
        Returns the 2D array of the AGN kernel map.
        The kernel map is a 2D array that represents the morphology of the source.
        The kernel map is used to convolve with the microlensing magnification map.
        The kernel is normalized to 1.
...
class SupernovaeSourceMorphology(SourceMorphology):
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        super().__init__(*args, **kwargs)
        raise NotImplementedError("Supernovae source morphology is not implemented yet.")