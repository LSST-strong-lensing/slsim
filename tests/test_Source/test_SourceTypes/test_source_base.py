...
class TestSourceBase:
    def setup_method(self):
        ...
        self.source_dict2 = {"z": [0.7], "mag_i": [23], "n_sersic": [1],
                                         "angular_size": [0.2], "e1": [0.002], "e2": [0.004]}
        self.source_dict3 = {"z": 0.8, "mag_i": 23, "n_sersic": 1,
                            "angular_size": 0.2, "e1": 0.002, "e2": 0.004, 
                    "center_x": 0.035, "center_y": 0.044, "ra_off": 0.001, "dec_off": 0.002}
    ...
    def test_source_position(self):
        x, y = self.source.extended_source_position(reference_position=[0, 0],
                                                     draw_area=4*np.pi)
        x2, y2 = self.source.point_source_position(reference_position=[0, 0],
                                                     draw_area=4*np.pi)
        x3, y3 = self.source3.extended_source_position(reference_position=[0, 0],
                                                     draw_area=4*np.pi)
        x4, y4 = self.source3.point_source_position(reference_position=[0, 0],
                                                     draw_area=4*np.pi)
        assert -2 < x < 2
        assert -2 < y < 2
        assert -2 < x2 < 2
