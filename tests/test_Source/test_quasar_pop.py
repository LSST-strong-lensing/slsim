...
class TestQuasarRate:
    ...
    def test_convert_magnitude(self):
        ...
        expected_abs_mags = [-25.40439324, -27.86579516, -23.21158492, -25.02167014, -25.15264705]
        expected_app_mags = test_magnitudes

        # Test case #1: Apparent to Absolute Magnitude Conversion
        for z, mag, expected_abs_mag in zip(test_redshifts, test_magnitudes, expected_abs_mags):
            abs_mag = self.quasar_rate.convert_magnitude(mag, z, conversion='apparent_to_absolute')
            np.testing.assert_almost_equal(abs_mag, expected_abs_mag, decimal=4)

        # Test case #2: Absolute to Apparent Magnitude Conversion
        for z, expected_abs_mag, expected_app_mag in zip(test_redshifts, expected_abs_mags, expected_app_mags):
            app_mag = self.quasar_rate.convert_magnitude(expected_abs_mag, z, conversion='absolute_to_apparent')
    ...
    def test_generate_quasar_redshifts(self):
        ...
        assert isinstance(sampled_redshifts, np.ndarray), f"Returned object is not a numpy array, got {type(sampled_redshifts)} instead."
    
        single_redshift = self.quasar_rate.generate_quasar_redshifts(m_min=15, m_max=25, return_single=True)
        assert isinstance(single_redshift, float), f"Returned object is not a float, got {type(single_redshift)} instead."

        assert sampled_redshifts.size > 0, "No redshifts were generated."

    def test_compute_cdf_data(self):
        sampled_redshifts = self.quasar_rate.generate_quasar_redshifts(m_min=15, m_max=25)
        cdf_data_dict = self.quasar_rate.compute_cdf_data(m_min=15, m_max=25, quasar_redshifts=sampled_redshifts)

        assert isinstance(cdf_data_dict, dict), "Output should be a dictionary."
            
        computed_redshifts = list(cdf_data_dict.keys())
        assert np.allclose(computed_redshifts, np.unique(sampled_redshifts)), "Keys should match the redshift values."
        ...
        for z, (sorted_M_values, cumulative_prob_norm) in cdf_data_dict.items():
            ...
            assert np.all((cumulative_prob_norm >= 0) & (cumulative_prob_norm <= 1)), "Cumulative probabilities should be between 0 and 1."
                
            assert np.isclose(cumulative_prob_norm[-1], 1.0), "Cumulative probabilities should end at 1."
                
            assert len(sorted_M_values) == len(cumulative_prob_norm), "Lengths of sorted M values and cumulative probabilities should match."
    ...
    def test_inverse_cdf_fits_for_redshifts_ks(self):
        ...
        inverse_cdf_dict = self.quasar_rate.inverse_cdf_fits_for_redshifts(m_min, m_max, quasar_redshifts)
        ...
        theoretical_cdf_values = np.arange(1, len(theoretical_samples) + 1) / len(theoretical_samples)

        empirical_samples = np.sort(theoretical_samples)
        empirical_cdf_values = np.arange(1, len(empirical_samples) + 1) / len(empirical_samples)
    ...
    def test_quasar_sample(self):
        ...
        assert isinstance(table, Table), f"Returned object is not an Astropy Table. Type: {type(table)}"
