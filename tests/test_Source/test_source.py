...
class TestSource():
    def setup_method(self):
        self.source_dict = {'z': 0.5,
            'mag_r': 20.0,
            'mag_g': 18.0, 'mag_i': 21.0
        }
    ...
    def test_magnitude_with_variability(self):
        ...
        mag = source.magnitude('r', magnification=np.array([2.0, -0.5]), 
                        image_observation_times=np.array([[0.0, 1.0], [2, 3]])*u.day)
        mag_cal=np.array([[19.24742501, 19.24742501],
                [20.75257499, 20.75257499]])
    ...
    def test_magnitude_invalid_variability_model(self):
        ...
        with pytest.raises(ValueError):
            source.magnitude('r', magnification=np.array([2.0, -0.5]), 
                        image_observation_times=np.array([[0.0, 1.0], [2, 3]])*u.day)
