...
def test_dPhi_dM():
    ...
    expected_denominator = (10**(0.4 * (alpha + 1) * (M - M_star_value))) + (10**(0.4 * (beta + 1) * (M - M_star_value)))
    ...
    alpha_val = -2.58
    M_star_value = number_density_quasar.M_star(z_value)
    expected_denominator = (10**(0.4 * (alpha_val + 1) * (M - M_star_value))) + (10**(0.4 * (beta + 1) * (M - M_star_value)))
    if expected_denominator == 0:
        expected_value = np.nan
    else:
...
def test_inverse_cdf_fits_for_redshifts():
    ...
    inverse_cdf_dict = number_density_quasar.inverse_cdf_fits_for_redshifts(M_values, random_redshift_values)
    ...
    for i, z_value in enumerate(random_redshift_values):
        ...
        cumulative_probabilities = np.cumsum([number_density_quasar.dPhi_dM(M, z_value) for M in M_values_sorted])
...
def test_generate_redshift_table():
    ...
    table = number_density_quasar.generate_redshift_table(random_redshift_values, inverse_cdf_dict)
