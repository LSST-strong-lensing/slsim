...
def test_background_native_bands(monkeypatch, catalog_type, count, enabled):
    ...
    source = CatalogSource(
        angular_size=0.3, e1=0.1, e2=0.0, n_sersic=1,
        cosmo=FlatLambdaCDM(H0=70, Om0=0.3), catalog_type=catalog_type,
        catalog_path="unused", subtract_background=enabled,
        z=1, mag_i=22, mag_r=22, center_x=0, center_y=0,
    )
    ...
    monkeypatch.setattr(source, "_match_source", lambda **kw: (images, 0.03, 0, {"NOISE_MEAN": 2.0}))
