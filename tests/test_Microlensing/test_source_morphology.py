...
def kwargs_AGN_wave(cosmology):
    """Provides a dictionary of parameters for the AGN source using wavelength."""
    resolution = 100
    return {
        "source_redshift": 0.1,
        "cosmo": cosmology,
...
class TestSourceMorphology:
    """Tests the SourceMorphology base class methods indirectly and directly."""
    ...
    def test_base_class_pixel_scale_attribute_error(self):
        base_source = SourceMorphology()
        with pytest.raises(AttributeError): _ = base_source.length_x
        with pytest.raises(AttributeError): _ = base_source.pixel_scale_x
        with pytest.raises(AttributeError, match="Pixel scale not defined."):
            _ = base_source.pixel_scale
        with pytest.raises(AttributeError): _ = base_source.pixel_scale_x_m
        with pytest.raises(AttributeError): _ = base_source.pixel_scale_y_m
        with pytest.raises(AttributeError): _ = base_source.pixel_scale_m
    ...
    def test_conversions(self, gaussian_source, cosmology):
        ...
        arcsecs_val_round_trip = gaussian_source.metres_to_arcsecs(metres_val, cosmology, redshift)
    ...
    def test_property_caching(self, gaussian_source, agn_source_wave): # Use fixture
        """Tests that properties like kernel_map and derived scales are cached."""
        # --- Test Gaussian kernel_map caching ---
        assert hasattr(gaussian_source, "_kernel_map")
        initial_kernel = gaussian_source.kernel_map.copy()
        with patch.object(GaussianSourceMorphology, "get_kernel_map", wraps=gaussian_source.get_kernel_map) as mock_get_kernel:
        ...
        with patch.object(AGNSourceMorphology, "arcsecs_to_metres", wraps=local_agn_source.arcsecs_to_metres) as mock_arcsecs_to_metres:
            if hasattr(local_agn_source, "_pixel_scale_x_m"): del local_agn_source._pixel_scale_x_m
            if hasattr(local_agn_source, "_pixel_scale_y_m"): del local_agn_source._pixel_scale_y_m
            if hasattr(local_agn_source, "_pixel_scale_m"): del local_agn_source._pixel_scale_m
...
class TestGaussianSourceMorphology:
    ...
    def test_get_kernel_map(self, gaussian_source, kwargs_Gaussian):
        ...
        assert kernel.shape == (kwargs_Gaussian["num_pix_y"], kwargs_Gaussian["num_pix_x"])
    ...
    def test_properties(self, gaussian_source, kwargs_Gaussian):
        ...
        if pix_scale_x_m > 0 and pix_scale_y_m > 0:
             np.testing.assert_allclose(pix_scale_m, np.sqrt(pix_scale_x_m * pix_scale_y_m), rtol=1e-6)

        expected_pix_scale_x_m = gaussian_source.arcsecs_to_metres(
            gaussian_source.pixel_scale_x, gaussian_source.cosmo, gaussian_source.source_redshift
        )
...
class TestAGNSourceMorphology:
    ...
    def test_initialization_band(self, agn_source_band, kwargs_AGN_band):
        ...
        assert agn_source_band.observing_wavelength_band == kwargs_AGN_band["observing_wavelength_band"]
        ...
        np.testing.assert_allclose(agn_source_band.observer_frame_wavelength_in_nm, expected_wave_nm)
        ...
        assert kernel.shape[0] > 0 and kernel.shape[1] > 0 # Basic check for valid shape

        # Check scales were calculated
        assert hasattr(agn_source_band, "_pixel_scale_x_m") and agn_source_band.pixel_scale_x_m > 0
        assert hasattr(agn_source_band, "_pixel_scale_y_m") and agn_source_band.pixel_scale_y_m > 0
        assert hasattr(agn_source_band, "_pixel_scale_m") and agn_source_band.pixel_scale_m > 0
        assert hasattr(agn_source_band, "_pixel_scale_x") and agn_source_band.pixel_scale_x > 0
        assert hasattr(agn_source_band, "_pixel_scale_y") and agn_source_band.pixel_scale_y > 0
        assert hasattr(agn_source_band, "_pixel_scale") and agn_source_band.pixel_scale > 0
    ...
    def test_initialization_wavelength(self, agn_source_wave, kwargs_AGN_wave):
        ...
        assert agn_source_wave.observer_frame_wavelength_in_nm == kwargs_AGN_wave["observer_frame_wavelength_in_nm"]
        ...
        assert hasattr(agn_source_wave, "_pixel_scale_x_m") and agn_source_wave.pixel_scale_x_m > 0
        assert isinstance(agn_source_wave.length_x, float) and agn_source_wave.length_x > 0
        # Check num_pix property matches actual kernel shape
    ...
    def test_properties(self, agn_source_wave, kwargs_AGN_wave):
        """Tests the property accessors of the AGN source and scale consistency."""
        ...
        expected_pix_scale_x_m = 2 * kwargs_AGN_wave["r_out"] * grav_radius_val / kernel_shape[1]
        expected_pix_scale_y_m = 2 * kwargs_AGN_wave["r_out"] * grav_radius_val / kernel_shape[0]
        ...
        np.testing.assert_allclose(agn_source_wave.pixel_scale_x_m, expected_pix_scale_x_m, rtol=1e-6)
        np.testing.assert_allclose(agn_source_wave.pixel_scale_y_m, expected_pix_scale_y_m, rtol=1e-6)
        np.testing.assert_allclose(agn_source_wave.pixel_scale_m, expected_pix_scale_m, rtol=1e-6)

        # Test conversion back to arcsec using property access
        expected_pix_scale_x = agn_source_wave.metres_to_arcsecs(
            expected_pix_scale_x_m, agn_source_wave.cosmo, agn_source_wave.source_redshift
        )
        expected_pix_scale_y = agn_source_wave.metres_to_arcsecs(
            expected_pix_scale_y_m, agn_source_wave.cosmo, agn_source_wave.source_redshift
        )
        expected_pix_scale = np.sqrt(expected_pix_scale_x * expected_pix_scale_y)

        np.testing.assert_allclose(agn_source_wave.pixel_scale_x, expected_pix_scale_x, rtol=1e-6)
        np.testing.assert_allclose(agn_source_wave.pixel_scale_y, expected_pix_scale_y, rtol=1e-6)
        np.testing.assert_allclose(agn_source_wave.pixel_scale, expected_pix_scale, rtol=1e-6)
        ...
        np.testing.assert_allclose(agn_source_wave.length_x, expected_length_x, rtol=1e-6)
        np.testing.assert_allclose(agn_source_wave.length_y, expected_length_y, rtol=1e-6)

    def test_agn_not_implemented_methods(self, agn_source_wave):
        """Tests that specific AGN methods raise NotImplementedError."""
        with pytest.raises(NotImplementedError, match="This method is not implemented yet."):
...
class TestSupernovaeSourceMorphology:
    ...
    def test_init_not_implemented(self):
        """Tests that initializing SupernovaeSourceMorphology raises NotImplementedError."""
        with pytest.raises(NotImplementedError, match="Supernovae source morphology is not implemented yet."):
