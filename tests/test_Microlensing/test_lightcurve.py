...
def dummy_magmap():
    ...
    magmap2D_path = os.path.join(os.path.dirname(__file__), "TestData", "test_magmap2D.npy")
...
def mlc_instance():
    ...
    return MicrolensingLightCurve(magnification_map=dummy_magmap(), 
...
class TestMicrolensingLightCurve:
    ...
    def test_get_convolved_map(self, mlc_instance):
        ...
        convolved_map = mlc_instance._get_convolved_map(source_size=source_size, 
                                                        return_source_kernel=False)
        ...
        assert np.max(convolved_map) <= np.max(mlc_instance.magnification_map.magnifications)

        # Test with returning kernel
        convolved_map_2, source_kernel = mlc_instance._get_convolved_map(source_size=source_size, return_source_kernel=True)
        ...
        assert np.isclose(np.sum(source_kernel), 1.0, atol=1e-6) # Kernel should sum to 1
        assert np.all(convolved_map == convolved_map_2) # Result should be same


    def test_generate_point_source_lightcurve_basic(mlc_instance, 
                                                    cosmo,
                                                    source_redshift,
                                                    source_size,
                                                    source_velocity,):
        ...
        expected_length = int(source_velocity_mps * expected_time_seconds / pixel_size_meter) + 2
