...
def base_init_kwargs(lens_source_info, microlensing_params, cosmology):
    """Provides the standard dictionary to initialize MicrolensingLightCurveFromLensModel."""
    ...
    return {
        ...
        "kwargs_magnification_map": {
            ...
            "num_pixels_x": 50, "num_pixels_y": 50,
        },
...
def saved_magmaps(microlensing_params, lens_source_info):
    """Loads actual saved .npy maps from disk and constructs real MagnificationMap instances."""
    num_images = len(microlensing_params["kappa_star"])
    loaded_magmaps = []
    
    test_dir = os.path.dirname(os.path.abspath(__file__))
    saved_map_directory = os.path.join(test_dir, "..", "TestData", "test_magmaps_microlensing")
    ...
    for i in range(num_images):
        ...
        map_kwargs = {
            "theta_star": theta_star,
            "center_x": 0, "center_y": 0,
            "half_length_x": 2.5 * theta_star, "half_length_y": 2.5 * theta_star,
            "num_pixels_x": 50, "num_pixels_y": 50,
            "kappa_tot": microlensing_params["kappa_tot"][i],
            "shear": microlensing_params["shear"][i],
            "kappa_star": microlensing_params["kappa_star"][i]
        }
        loaded_magmaps.append(MagnificationMap(magnifications_array=mag_data, **map_kwargs))
...
def safe_ml_model(base_init_kwargs):
    """Returns an initialized model with a safe starting track to avoid map boundaries."""
...
class TestKinematicsAndTracking:
    ...
    def test_effective_velocity_returns_correct_shapes(self, safe_ml_model, microlensing_params):
        num_images = len(microlensing_params["kappa_star"])
        velocities, angles = safe_ml_model._effective_transverse_velocity_images(random_seed=42)
        
        assert velocities.shape == (num_images,)
        assert angles.shape == (num_images,)
        assert np.all(velocities >= 0)

    def test_effective_velocity_non_magmap_frame(self, safe_ml_model, microlensing_params):
        num_images = len(microlensing_params["kappa_star"])
    ...
    def test_astropy_quantity_handling_in_velocities(self, base_init_kwargs, microlensing_params):
        args = base_init_kwargs.copy()
        args["ra_lens"] *= u.deg
        args["dec_lens"] *= u.deg
    ...
    def test_effective_velocity_pole_branch(self, base_init_kwargs):
        """Forces the cross-product to hit the np.allclose(e1, 0) branch by putting the lens at DEC=90."""
        args = base_init_kwargs.copy()
    ...
    def test_effective_velocity_property_caching(self, base_init_kwargs):
        ...
        We use a clean model here because safe_ml_model manually overwrites the cache."""
        model = MicrolensingLightCurveFromLensModel(**base_init_kwargs)
    ...
    def test_explicit_start_position_reset(self, safe_ml_model):
        new_pos = safe_ml_model.reset_start_position(x_start_position=1e-6, y_start_position=2e-6)
    ...
    def test_random_start_position_reset(self, safe_ml_model):
        """Calling reset without values should randomly generate a valid start."""
...
class TestLightcurveGeneration:
    ...
    def test_magmaps_images_caching(self, mock_magmap, base_init_kwargs, microlensing_params):
        """Tests that generating magmaps is properly cached by running the real method
        and intercepting the underlying MagnificationMap class initialization."""
        model = MicrolensingLightCurveFromLensModel(**base_init_kwargs)
    ...
    def test_generate_point_source_lightcurves_structure(
        ...
        lightcurves, tracks, time_arrays = safe_ml_model.generate_point_source_lightcurves(
    ...
    def test_generate_magnitudes_array_returns_correct_shape(
        ...
        magnitudes = safe_ml_model.generate_point_source_microlensing_magnitudes(time_array)
    ...
    def test_generate_magnitudes_list_time(
        ...
        magnitudes = safe_ml_model.generate_point_source_microlensing_magnitudes(time_list)
        assert magnitudes.shape == (num_images, 50)

    def test_generate_magnitudes_scalar_time(
        self, patched_magmap_generation, safe_ml_model, microlensing_params
    ):
        """Tests the branch where time is provided as a single scalar (int/float)."""
        num_images = len(microlensing_params["kappa_star"])
    ...
    def test_invalid_time_format_raises_error(self, safe_ml_model):
        with pytest.raises(ValueError, match="Time array not provided in the correct format"):
    ...
    def test_lightcurves_and_tracks_cached_after_generation(
        ...
        """Ensures that the @property methods for tracks and lightcurves work and raise appropriately."""
