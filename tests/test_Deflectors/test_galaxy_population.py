...
def test_galaxy_size():
    ...
    calculated_size_model2 = galaxy_size(
        mh, mstar, z, cosmo, q_out, model2, scatter=False, sig_tb=sig_tb
    )  # in units of Mpc/h
    assert (
        0 < calculated_size_model2 < 0.01
    ), "The galaxy size should be less than 10 kpc"
    ...
    calculated_size_model3 = galaxy_size(
        mh, mstar, z, cosmo, q_out, model3, scatter=False
    )  # in units of Mpc/h
    assert (
        0 < calculated_size_model3 < 0.01
    ), "The galaxy size should be less than 10 kpc"
