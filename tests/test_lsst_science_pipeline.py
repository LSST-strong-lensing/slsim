...
from slsim.lsst_science_pipeline import (variable_lens_injection, 
...
def test_variable_lens_injection(pes_lens_instance):
    ...
    results = variable_lens_injection(lens_class, band='i', delta_pix=0.2, 
                        num_pix = 301, transform_pix2angle=transform_matrices
...
def test_multiple_variable_lens_injection(pes_lens_instance):
    ...
    for data in expo_data:
        transform_matrices.append([transf_matrix_single.copy() 
                                    for _ in range(len(data))])
    results = multiple_variable_lens_injection(lens_class, band='i', delta_pix=0.2, 
                        num_pix = 301, transform_matrices_list=transform_matrices
