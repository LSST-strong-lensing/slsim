...
from slsim.Util.cosmo_util import (
    merge_catalogs,
    match_simulated_to_real,
    flux_weighted_center,
)
...
def test_merge_catalogs_basic():
    """Test that merge_catalogs merges matching entries and computes MAG_DIFF
    correctly."""
    ...
    merged = merge_catalogs(
        large, small, lc_mag_col="MAG", sc_mag_col="MAG", tolerance=1.0
    )
    assert len(merged) == 1
...
def test_match_simulated_to_real_basic(tmp_path):
    """Test match_simulated_to_real with a minimal simulated table and real
    catalog."""
    sim = Table()
    sim["mag_i"] = [20.0, 21.0]
    sim["z"] = [0.5, 1.0]
...
def test_flux_weighted_center_zero():
    """Test that flux_weighted_center returns image center if all pixels are
    zero."""
    img = np.zeros((10, 20))
...
def test_fingerprint_consistency(tmp_path):
    """Test that _fingerprint yields consistent hashes for same inputs and
...
def test_normalise():
    """Test that _normalise scales arrays to [0, 1], and handles constant
    arrays."""
