...
def test_false_positive():
    ...
    lens = Deflector(deflector_type="EPL",
                        deflector_dict=single_deflector)
    ...
    source_list=[Source(source_dict=single_source1,
                    cosmo=cosmo,
                    source_type="extended",
                    light_profile="single_sersic"), Source(source_dict=single_source2,
                    cosmo=cosmo,
                    source_type="extended",
                    light_profile="single_sersic")]
    # LOS configuration
    ...
    assert false_positive_instance_1.lenstronomy_kwargs("i")[0][
        "lens_light_model_list"][0] == 'SERSIC_ELLIPSE'
    assert len(false_positive_instance_2.lenstronomy_kwargs("i")[0][
        "lens_light_model_list"]) == 3
    assert len(false_positive_instance_2.lenstronomy_kwargs("i")[1][
        "kwargs_lens_light"]) == 3
    ...
    assert false_positive_instance_1.extended_source_magnitude(band="i") ==\
