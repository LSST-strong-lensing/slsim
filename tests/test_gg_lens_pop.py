...
class TestGGLensPop:
    ...
    def test_num_lenses_and_sources(self, gg_lens_pop_instance):
        ...
        assert 100 <= num_lenses <= 6600, f"Expected num_lenses to be between 5800 and 6600," 
        f"but got {num_lenses}"
        assert 100000 <= num_sources <= 500000, f"Expected num_sources to be between 1090000 and" 
    ...
    def test_num_sources_tested_and_test_area(self, gg_lens_pop_instance):
        ...
        assert 0.01 < test_area < 100 * np.pi, f"Expected test_area to be between 0.1 and 100*pi,"
        f"but got {test_area}"
        num_sources_range = gg_lens_pop_instance.get_num_sources_tested(testarea=test_area)
        assert 0 <= num_sources_range <= 50, f"Expected num_sources_range to be between 0 and 50,"
