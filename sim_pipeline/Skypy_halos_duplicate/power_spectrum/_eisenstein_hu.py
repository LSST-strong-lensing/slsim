...
This implements the Eisenstein and Hu fitting formula for the matter power spectrum.
"""
...
def transfer_with_wiggles(wavenumber, A_s, n_s, cosmology, kwmap=0.02):
    r"""Eisenstein & Hu transfer function with wiggles. This function returns the
    Eisenstein & Hu fitting formula for the transfer function with baryon acoustic
    oscillation wiggles. This is described in [1]_ and [2]_.
    ...
    if not ob0:
        raise ValueError(
            "Ob0 for input cosmology must be non-zero if " "wiggles = True"
        )
    ...
    if not Tcmb0:
        raise ValueError(
            "Tcmb0 for input cosmology must be non-zero if" "wiggles = True"
        )
    ...
    beta_b = 0.5 + f_baryon + (3 - 2 * f_baryon) * np.sqrt((17.2 * om0h2) ** 2 + 1.0)
    ...
    T_c = T_c_f * f(T_c_ln_beta, T_c_C_noalpha) + (1 - T_c_f) * f(
...
def transfer_no_wiggles(wavenumber, A_s, n_s, cosmology):
    r"""Eisenstein & Hu transfer function without wiggles. Eisenstein & Hu fitting
    formula for the transfer function without baryon acoustic oscillation wiggles. This
    is described in [1]_ and [2]_.
    ...
    alpha = (
        1
        - 0.328 * np.log(431 * om0h2) * f_baryon
        + 0.38 * np.log(22.3 * om0h2) * f_baryon**2
    )
    sound = 44.5 * np.log(9.83 / om0h2) / np.sqrt(1 + 10 * (f_baryon * om0h2) ** (0.75))
    shape = om0h2 * (alpha + (1 - alpha) / (1 + (0.43 * wavenumber * sound) ** 4))
    aq = wavenumber * (Tcmb0 / 2.7) ** 2 / shape
    transfer = np.log(2 * np.e + 1.8 * aq) / (
        np.log(2 * np.e + 1.8 * aq) + (14.2 + 731 / (1 + 62.5 * aq)) * aq * aq
    )
...
def eisenstein_hu(wavenumber, A_s, n_s, cosmology, kwmap=0.02, wiggle=True):
    """Eisenstein & Hu matter power spectrum. This function returns the Eisenstein and
    Hu fitting function for the linear matter power spectrum with (or without) baryon
    acoustic oscillations, c.f. [1]_ and [2]_, using formulation from Komatsu et al
    (2009) in [3]_.
    ...
    if wiggle:
        transfer = transfer_with_wiggles(wavenumber, A_s, n_s, cosmology, kwmap)
    else:
    ...
    power_spectrum = (
        A_s
        * (2 * (wavenumber / H0_c) ** 2 / 5 / om0) ** 2
        * transfer**2
        * (wavenumber / kwmap) ** (n_s - 1)
        * 2
        * np.pi**2
        / (wavenumber) ** 3
    )
