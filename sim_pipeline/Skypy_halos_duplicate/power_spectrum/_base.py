...
class TabulatedPowerSpectrum(PowerSpectrum):
    ...
    def __init__(self, wavenumber, redshift, power_spectrum):
        kx = min(
            np.size(redshift) - 1,
            signature(RectBivariateSpline).parameters["kx"].default,
        )
        ky = min(
            np.size(wavenumber) - 1,
            signature(RectBivariateSpline).parameters["ky"].default,
        )
        self.interpolator = RectBivariateSpline(
            redshift, np.log10(wavenumber), np.log10(power_spectrum), kx=kx, ky=ky
        )
    ...
    def __call__(self, wavenumber, redshift):
        ...
        interp = self.interpolator(
            np.atleast_1d(redshift)[sort_z], np.log10(np.atleast_1d(wavenumber)[sort_k])
        )
